...
Var
 ...
 VValorFinal: Double;
begin
  VPercentual := FUCtrlVendas.CdsTipoDescontos.FieldByName('PERCENTUAL').AsInteger;
  FUCtrlVendas.ValorDesconto := ((FUCtrlVendas.ValorBrutpo * VPercentual) /100);
  VValorFinal := FUCtrlVendas.ValorBrutpo - FUCtrlVendas.ValorDesconto;
  LbValorLiquido.Caption := 'R$ '+ TFuncionalidades.FormatarValorTexto(VValorFinal);
  LbValorDescontp.Caption := 'R$ '+ TFuncionalidades.FormatarValorTexto(FUCtrlVendas.ValorDesconto);
...
begin
  ...
  begin
    ValorButo := ValorButo + FUCtrlVendas.QueryItensVenda.FieldByName('VALOR_SUBTOTAL').AsFloat;
    FUCtrlVendas.QueryItensVenda.Next;
  end;
  FUCtrlVendas.ValorBrutpo := ValorButo;
...
begin
  ...
  MessageDlg('Pagamento Relazado!',TMsgDlgType.mtConfirmation,[TMsgDlgBtn.mbOK],0);
  BtnSairPagto.Click;
  Btn_Fechar.Click;
end;
