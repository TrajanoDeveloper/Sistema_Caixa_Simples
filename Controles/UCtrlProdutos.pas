...
type
  TUCtrlProdutos = class
    ...
    public
      ...
      procedure CarregarComboGrupoProdutos;
      ...
      function BuscarValorProduto(PcodigoProduto: integer): Double;
  end;

implementation
{ TUCtrlProdutos }

function TUCtrlProdutos.BuscarValorProduto(PcodigoProduto: integer): Double;
begin
  QueryAux.close;
  QueryAux.sql.Clear;
  QueryAux.SQL.Add('SELECT VALOR FROM CADPRODUTO WHERE CODIGO = '+IntToStr(PcodigoProduto) );
  QueryAux.Open;

  if not QueryAux.IsEmpty then
    Result := QueryAux.FieldByName('VALOR').AsFloat
  else
    Result := 0;
end;

procedure TUCtrlProdutos.CarregarComboGrupoProdutos;
begin
...
begin
  ...
  QueryProduto.SQL.Add('    CP.CODIGO,                                                              ');
  QueryProduto.SQL.Add('    CP.DESCRICAO,                                                           ');
  QueryProduto.SQL.Add('    CP.CODIGO_GRUPO,                                                        ');
  QueryProduto.SQL.Add('    REPLACE(CAST(CP.VALOR AS VARCHAR(10)), ''.'', '','') as VALOR_FORMATADO,');
  QueryProduto.SQL.Add('    CP.VALOR,                                                               ');
  QueryProduto.SQL.Add('    CG.DESCRICAO AS DESC_GRUPO                                              ');
  QueryProduto.SQL.Add('FROM                                                                        ');
  QueryProduto.SQL.Add('  CADPRODUTO CP                                                             ');
  QueryProduto.SQL.Add('  JOIN CADGRUPOPRODUTO CG                                                   ');
  QueryProduto.SQL.Add('   ON CP.CODIGO_GRUPO = CG.CODIGO                                           ');
  QueryProduto.SQL.Add('  ORDER BY CP.CODIGO                                                        ');
  QueryProduto.Open;
end;
    ...
    Incluir:
      begin
        VSql := 'INSERT INTO CADPRODUTO                                                            '+
                ...
                TFuncionalidades.FormatarTextoValor(QueryProduto.FieldByName('VALOR').AsString)+') ';
    ...
    Alterar:
      begin
        VSql := 'UPDATE CADPRODUTO SET                                                                       '+
                ...
                ' VALOR = '+ TFuncionalidades.FormatarTextoValor(QueryProduto.FieldByName('VALOR').AsString)  +
    ...
    Excluir:
      begin
        if PCodigoGrupoProduto > 0 then
          ...
          VSql := 'DELETE FROM CADPRODUTO WHERE CODIGO = '+
